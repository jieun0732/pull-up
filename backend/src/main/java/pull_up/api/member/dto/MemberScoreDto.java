...
public record MemberScoreDto (
    ...
    public static MemberScoreDto of(Long id, String name, String email, boolean accessCheck, String role, List<Exam> scores) {

        // 가장 최근의 ExamInformation의 Score를 가져오기
        Integer latestScore = scores == null? Integer.valueOf(0) : scores.stream()
                .max(Comparator.comparing(Exam::getCreatedDate)) // 가장 최근의 ExamInformation 찾기
                .map(Exam::getScore) // Score 반환
