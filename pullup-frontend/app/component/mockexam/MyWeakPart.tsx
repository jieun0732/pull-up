...
function MyWeakPart() {
  ...
  const [weakestPart, setWeakestPart] = useState<ProblemTypeResult[]>([]);
    ...
    const displayWeakPart = weakestPart.length > 0 ? weakestPart : [
  ...
  useEffect(() => {
    if (averageScore) {
      ...
      const updatedWeakPart = averageScore.problemTypeResults.map((item) => ({
        ...item,
        weakest: false, // 초기화
        errorRate: (item.totalProblems - item.correctProblems) / item.totalProblems, // 오답 비율
      }));

      // 가장 오답 비율이 높은 부분 찾기
      const maxErrorRate = Math.max(...updatedWeakPart.map((item) => item.errorRate || 0));
    ...
    return (
      ...
      <div className="flex w-full justify-around">
        {displayWeakPart.map((item, index) => {
          ...
          return (
            <div key={index} className="flex w-14 flex-col items-center justify-end gap-1">
              {item.weakest && (
              ...
              <div className="relative h-32 w-8 rounded-md bg-red02">
                <div className="absolute bottom-0 left-0 z-20 w-8 rounded-md bg-red01" style={{ height }}></div>
              </div>
              <Text size="caption-02" color="text-gray01">
                {item.entry || "??"}
              </Text>
  ...
  return (
    <div className="my-5 flex w-full flex-col rounded-2xl bg-white p-6">
      ...
      {weakestPart.length > 0 && (
        <Text size="head-02" className="inline">
          <span className="text-red01"> {weakestPart.find((part) => part.weakest)?.entry}</span> 이에요!
        </Text>
      )}
      <Text size="caption-02" color="text-gray01" className="mt-1">
        * 오답 개수 기준이에요.
      ...
      <div className="flex w-full justify-around">
        {weakestPart.map((item) => {
          const height = `${(item.correctProblems / item.totalProblems) * 100}%`;
          return (
            <div key={item.entry} className="flex w-14 flex-col items-center justify-end gap-1">
              {item.weakest && (
                <div className="relative mb-1 mt-6 flex h-7 w-14 items-center justify-center rounded-[.4em] bg-[#3d4150] text-center text-[11px] text-white">
                  취약파트
                </div>
